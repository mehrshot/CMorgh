...
void saveState(vector<vector<string>>& historyStack, const vector<string>& lines) {
    historyStack.push_back(lines);  // Save the current state
}

void undo(vector<vector<string>>& historyStack, vector<vector<string>>& redoStack, vector<string>& lines) {
    if (!historyStack.empty()) {
        redoStack.push_back(lines);  // Save current state for redo
        lines = historyStack.back(); // Restore previous state
        historyStack.pop_back();     // Remove the restored state from history
    }
}

void redo(vector<vector<string>>& historyStack, vector<vector<string>>& redoStack, vector<string>& lines) {
    if (!redoStack.empty()) {
        historyStack.push_back(lines);  // Save current state for undo
        lines = redoStack.back();       // Restore undone state
        redoStack.pop_back();           // Remove the restored state from redo stack
    }
}
...
//-----Selection--
//----------------
void deleteSelection(bool& selectionActive, int& selectionStartLine, int& currentLine,
                      vector<string>& lines, int& cursorPos, int selectionStartPos) {
    if (!selectionActive) return;

    int startLine = min(selectionStartLine, currentLine);
    int endLine = max(selectionStartLine, currentLine);
    int startPos = (selectionStartLine == startLine) ?
                   selectionStartPos : cursorPos;
    int endPos = (currentLine == endLine) ?
                 cursorPos : selectionStartPos;

    if (startLine == endLine) {
        lines[startLine].erase(min(startPos, endPos), abs(endPos - startPos));
    } else {
        // Handle multi-line deletion
        string newLine = lines[startLine].substr(0, startPos) +
                         lines[endLine].substr(endPos);
        ...
        while (SDL_PollEvent(&e)) {
            if (e.type == SDL_QUIT) {
                ...
                if (SDL_GetModState() & KMOD_CTRL) {
                    saveState(historyStack, lines);
                    switch (e.key.keysym.sym) {
                        case SDLK_c: // COPY (Improved)
                        {
                            std::string selectedText = "";
                            // Ensure start and end positions are correct even if the user moved the cursor around
                            if (selectionActive)
                            {
                                int startLine = std::min(selectionStartLine, currentLine);
                                int endLine = std::max(selectionStartLine, currentLine);
                                int startPos = (startLine == selectionStartLine) ? selectionStartPos : 0;
                                int endPos = (endLine == currentLine) ? cursorPos : lines[endLine].length();

                                if (startPos > endPos && startLine == endLine)
                                {
                                    int t = endPos;
                                    endPos = startPos;
                                    startPos = t;
                                ...
                                // Now construct the selected text from the lines
                                for (int i = startLine; i <= endLine; ++i) {
                                    int lineStart = (i == startLine) ? startPos : 0;
                                    int lineEnd = (i == endLine) ? endPos : lines[i].length();
                                    selectedText += lines[i].substr(lineStart, lineEnd - lineStart); // Correct substring length
                                    if (i < endLine) {
                                        selectedText += "\n"; // Add newline
                                    }
                                }

                                // Finally, set the clipboard text with the selected content
                        ...
                        case SDLK_x: // CUT
                        {
                            ...
                            if (selectionActive) {
                                ...
                                std::vector<std::string> newLines; // Store the modified lines

                                for (int i = 0; i < lines.size(); ++i) {
                                    if (i < startLine || i > endLine) {
                                        newLines.push_back(lines[i]); // Keep lines outside the selection
                                    } else if (i == startLine) {
                                        newLines.push_back(lines[i].substr(0, startPos) + lines[i].substr(endPos));
                                        if (selectedText.empty()) {
                                            currentLine = i;
                                            cursorPos = startPos;
                                        }
                                    } else if (i == endLine && startLine != endLine) {
                                        newLines.push_back(lines[i].substr(endPos));
                                    } else if (i > startLine && i < endLine) {
                                        // Skip completely deleted lines
                                    }
                                }
                                for (int i = startLine; i <= endLine; ++i) {
                                    int lineStart = (i == startLine) ? startPos : 0;
                                ...
                                SDL_SetClipboardText(selectedText.c_str());
                                lines = newLines; // Update lines with modified vector
                        ...
                        {
                            if (selectionActive)
                                deleteSelection(selectionActive, selectionStartLine, currentLine, lines, cursorPos,
                                                selectionStartPos);
                            ...
                            if (clipboardText != nullptr) {
                                ...
                                if (selectionActive) {
                                    ...
                                    for (int i = startLine; i <= endLine; ++i) {
                                        ...
                                        int charsOnLineToDelete = 0;

                                        if (i == startLine)
                                            charsOnLineToDelete = lineLength - startPos;
                                        else if (i == endLine)
                                            charsOnLineToDelete = endPos;
                                        else
                                            charsOnLineToDelete = lineLength;

                                        charsToDelete += charsOnLineToDelete;
                                    }

                                    lines[startLine].replace(startPos, charsToDelete, clipboardText);
                                    currentLine = startLine;
                                    cursorPos = startPos + strlen(clipboardText);
                                    selectionActive = false;
                                } else {
                ...
                bool isMovementKey = (e.key.keysym.sym == SDLK_LEFT ||
                                      e.key.keysym.sym == SDLK_RIGHT ||
                                      e.key.keysym.sym == SDLK_UP ||
                                      e.key.keysym.sym == SDLK_DOWN);

                if (!(SDL_GetModState() & KMOD_SHIFT) && selectionActive) {
                    selectionActive = false;
                }

                // Handle non-shift key presses first and reset selection
                if (!(SDL_GetModState() & KMOD_SHIFT)) {
                    selectionActive = false; // Reset selection if shift is not pressed
                    ...
                    // Handle Shift key modifiers separately
                else if (SDL_GetModState() & KMOD_SHIFT) {
                    if (!selectionActive) { // Start selection only if it's not already active
                        selectionStartPos = cursorPos;
                        selectionStartLine = currentLine;
                        selectionActive = true; // SET SELECTION ACTIVE HERE!
                    }
                    switch (e.key.keysym.sym) {
                        case SDLK_LEFT:
                            if (cursorPos > 0) {
                        ...
                        case SDLK_UP:
                            if (currentLine > 0) {
                                currentLine--;
                                cursorPos = std::min((int) lines[currentLine].size(),
                                                     cursorPos); // Corrected min/max order
                            }
                            break;
                        case SDLK_DOWN:
                            if (currentLine < lines.size() - 1) {
                    ...
                    break; // Important: Break out of the switch after handling the arrow keys
                } else {
                    selectionActive = false; // Disable selection if Shift is not pressed.
                }
            }
               else if (e.type == SDL_TEXTINPUT) {
                    ...
                    // Save state before inserting text
                    saveState(historyStack, lines);
                    if (e.text.text) {
                        if (e.text.text[0] == ' ')
                            lines[currentLine].insert(cursorPos, " ");
                            ...
                            if (mouseX >= project.rect.x &&
                                ...
                                if (file.is_open()) {
                                    ...
                                    SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_ERROR, "Error",
                                                             ("Failed to open project: " + project.name).c_str(),
                                                             window);
                                }
                    ...
                    if (EditMenuVisible && mouseX >= 85 && mouseX <= 85 + 50 && mouseY >= 40 && mouseY <= 80) {
                        undo(historyStack, redoStack, lines);
                    }
                        // Check if "Redo" button is clicked
                    else if (EditMenuVisible && mouseX >= 85 && mouseX <= 85 + 50 && mouseY >= 80 && mouseY <= 120) {
                        redo(historyStack, redoStack, lines);
                    }
                    ...
                    //edit button
                    if (mouseX >= 60 && mouseX <= 100 && mouseY < 40) {
                        EditMenuVisible = !EditMenuVisible;  // Toggle the visibility of the menu
                    }
            ...
            if (isDarkMode) {
                SDL_SetRenderDrawColor(renderer, darkBackgroundColor.r, darkBackgroundColor.g, darkBackgroundColor.b,
                                       255);
                textColor = darkTextColor;
                navarcolor = 50;
                textCol = 255;
            } else {
                SDL_SetRenderDrawColor(renderer, lightBackgroundColor.r, lightBackgroundColor.g, lightBackgroundColor.b,
                                       255);
            ...
            if (selectionActive) {
                ...
                int startPos = selectionStartPos;
                int endPos = cursorPos;

                int topLine = std::min(startLine, endLine);
                int bottomLine = std::max(startLine, endLine);
                ...
                for (int line = topLine; line <= bottomLine; ++line) {
                    ...
                    if (line == startLine && line == endLine) {
                        leftX = std::min(startPos, endPos);
                        rightX = std::max(startPos, endPos);
                    } else if (line == startLine) {
                        leftX = (startLine < endLine) ? startPos : 0;
                        rightX = (startLine < endLine) ? lines[line].size() : startPos;
                    } else if (line == endLine) {
                        leftX = (startLine < endLine) ? 0 : endPos;
                        rightX = (startLine < endLine) ? endPos : lines[line].size();
                    } else {
                        leftX = 0;
                        rightX = lines[line].size();
                    }

                    leftX = std::max(0, std::min(leftX, (int) lines[line].size()));
                    rightX = std::max(0, std::min(rightX, (int) lines[line].size()));

                    int yPos = -scrollOffset + 50 + (line * LINE_HEIGHT);
                    if (yPos + LINE_HEIGHT <= 0 || yPos >= SCREEN_HEIGHT) continue;

                    int baseX = (line == 0) ? 240 : 250;
                    string lineText = lines[line].empty() ? " " : lines[line];

                    int startX, endX;

                    if (leftX > 0) {
                        int leftWidth;
                        TTF_SizeText(codefont, lineText.substr(0, leftX).c_str(), &leftWidth, nullptr);
                        startX = baseX + leftWidth;
                    } else {
                        startX = baseX;
                    }

                    int rightWidth;
                    TTF_SizeText(codefont, lineText.substr(0, rightX).c_str(), &rightWidth, nullptr);
                    endX = baseX + rightWidth;

                    if (endX > startX) {
                        SDL_Rect selectionRect = {startX, yPos, endX - startX, LINE_HEIGHT};
                        SDL_SetRenderDrawColor(renderer, 0, 100, 255, 50); // Blue color
            ...
            for (size_t i = 0; i < savedProjects.size(); i++) {
                SDL_Surface *surface = TTF_RenderText_Blended(font, savedProjects[i].name.c_str(),
                                                              {255, 255, 255, 255});
            ...
            if (saveProjectButtonPressed) {
                ...
                if (showSaveDialog(projectName, textCol)) {
                    if (saveProject(projectName, lines)) {
                        ...
                        SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_INFORMATION, "Warning",
                                                 "Error: Project with this name already exists.", window);
                    }
                } else {
                    SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_INFORMATION, "Info", "Save operation was canceled.",
                                             window);
                }
            ...
            // If the menu is visible, render the menu options for edit
            if (EditMenuVisible) {
                SDL_SetRenderDrawColor(renderer, navarcolor, navarcolor, navarcolor, 255);  // Light gray color
                SDL_Rect grayBackgroundRect = {80, 35, 60, 80}; // Position it below the top menu
                SDL_RenderFillRect(renderer, &grayBackgroundRect);

                SDL_Surface *undoSurface = TTF_RenderText_Blended(font, undoText.c_str(), textColor);
                SDL_Texture *undoTexture = SDL_CreateTextureFromSurface(renderer, undoSurface);
                SDL_Rect undoRect = {85, 40, undoSurface->w, undoSurface->h};
                SDL_RenderCopy(renderer, undoTexture, nullptr, &undoRect);
                SDL_FreeSurface(undoSurface);
                SDL_DestroyTexture(undoTexture);

                SDL_Surface *redoSurface = TTF_RenderText_Blended(font, redoText.c_str(), textColor);
                SDL_Texture *redoTexture = SDL_CreateTextureFromSurface(renderer, redoSurface);
                SDL_Rect redoRect = {85, 80, redoSurface->w, redoSurface->h};
                SDL_RenderCopy(renderer, redoTexture, nullptr, &redoRect);
                SDL_FreeSurface(redoSurface);
